...
export async function fileHistoryMakeSnapshot(
  ...
  updateFileHistoryState((state: FileHistoryState) => {
    try {
      ...
      logForDebugging(`FileHistory: Added snapshot for ${messageId}, tracking ${state.trackedFiles.size} files`);
 ...
 */
export async function fileHistoryGetDiffStats(state: FileHistoryState, messageId: UUID): Promise<DiffStats> {
  if (!fileHistoryEnabled()) {
    return undefined;
  }
...
export async function fileHistoryGetSessionFileDiffStats(state: FileHistoryState): Promise<SessionFileDiffStat[]> {
 ...
 */
export async function fileHistoryHasAnyChanges(state: FileHistoryState, messageId: UUID): Promise<boolean> {
  if (!fileHistoryEnabled()) {
    return false;
  }
 ...
 */
async function applySnapshot(state: FileHistoryState, targetSnapshot: FileHistorySnapshot): Promise<string[]> {
  const filesChanged: string[] = [];
  for (const trackingPath of state.trackedFiles) {
    try {
      const filePath = maybeExpandFilePath(trackingPath);
 ...
 */
async function computeDiffStatsForFile(originalFile: string, backupFileName?: string): Promise<DiffStats> {
  const filesChanged: string[] = [];
  let insertions = 0;
  let deletions = 0;
  try {
 ...
 */
function getBackupFileNameFirstVersion(trackingPath: string, state: FileHistoryState): BackupFileName | undefined {
    ...
    await Promise.allSettled(
      fileHistorySnapshots.map(async snapshot => {
        const backupEntries = Object.values(snapshot.trackedFileBackups).filter(
          (backup): backup is typeof backup & { backupFileName: string } => backup.backupFileName !== null,
        );
        ...
        const results = await Promise.allSettled(
          backupEntries.map(async ({ backupFileName }) => {
            ...
            try {
              ...
              logError(new Error(`FileHistory: Error hard linking backup file from previous session`));
 ...
 */
async function notifyVscodeSnapshotFilesUpdated(oldState: FileHistoryState, newState: FileHistoryState): Promise<void> {
