...
function readCallbackPort(): number {
  ...
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error(`Invalid OPENAI_OAUTH_CALLBACK_PORT: ${raw}. Expected a port between 1 and 65535.`);
  }
...
export const OPENAI_OAUTH_CONFIG = {
  ...
  CLIENT_ID: envOrDefault('OPENAI_OAUTH_CLIENT_ID', DEFAULT_OPENAI_CODEX_CLIENT_ID),
  ...
  MANUAL_REDIRECT_URI: envOrDefault('OPENAI_OAUTH_MANUAL_REDIRECT_URI', 'https://platform.openai.com/oauth/callback'),
