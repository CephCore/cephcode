...
- `/provider` - Manage AI provider and model (updates session immediately upon change)
- `/buddy` - Configure AI companion (Buddy)
- Feature-gated commands now available via env vars:
  - `KAIROS=1` → `/assistant`, `/brief`
  - `BRIDGE_MODE=1` → `/bridge`
  - `ULTRAPLAN=1` → `/ultraplan`
  - `VOICE_MODE=1` → `/voice` (requires audio-capture-napi, now installed)
- Model fetching from provider APIs via new providerModels service
- Provider config supports storing API keys per provider
- Fixed build error: duplicate variable `t10` in Stats component
- `/buddy` command now available for configuring your AI companion (Buddy)
