...
export class LogUpdate {
  ...
  render(prev: Frame, next: Frame, altScreen = false, decstbmSafe = true): Diff {
    ...
    if (
      ...
      const prevHeightAtNewWidth = calculateWrappedHeight(prev.screen, next.viewport.width);
      ...
      if (prevHeightAtNewWidth <= next.viewport.height) {
        return [{ type: 'clear', count: prevHeightAtNewWidth }, ...this.renderFullFrame(next)];
      }
    ...
    // Windows Terminal: skip DECSTBM hardware scroll optimization —
    // scrolling can leave stale visual fragments in the buffer. Full
    // re-render avoids the issue.
    const isWindowsTerminal = process.platform === 'win32' && !!process.env.WT_SESSION;
    if (altScreen && next.scrollHint && decstbmSafe && !isWindowsTerminal) {
    ...
    diffEach(prev.screen, next.screen, (x, y, removed, added) => {
      ...
      if (
        ...
        (!removed || removed.width === CellWidth.SpacerTail || removed.width === CellWidth.SpacerHead)
      ) {
        return;
      }
