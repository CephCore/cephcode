 ...
 *
 * H23: The original regex /\u001b\[([0-9]+;)*4(;[0-9]+)*m/ had false positives
 * matching 256-color codes like \x1b[38;5;4m (foreground blue) and \x1b[48;5;4m
 * (background blue) because the `4` at the color-index position was matched as an
 * underline. The callback now verifies that `4` is an actual SGR underline code
 * and not a color-index parameter of 38/48/58 sequences.
 */
export function stripUnderlineAnsi(content: string): string {
  ...
  (match) => {
    // H23: Exclude false positives where 4 is a color-index parameter
    // in extended color sequences (38;5;4 = fg blue, 48;5;4 = bg blue,
    // 58;5;4 = underline color blue) or true-color RGB params.
    if (match.includes('38;5;4') || match.includes('48;5;4') || match.includes('58;5;4')) return match
    // True-color: 38;2;R;G;B or 48;2;R;G;B — any of R/G/B could be 4.
    if (match.includes(';2;')) return match
    return ''
  },
);
}
