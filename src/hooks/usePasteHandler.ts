...
export function usePasteHandler({ onPaste, onInput, onImagePaste }: PasteHandlerProps): {
  ...
  const resetPasteTimeout = React.useCallback(
    (currentTimeoutId: ReturnType<typeof setTimeout> | null) => {
      ...
      return setTimeout(
        (setPasteState, onImagePaste, onPaste, setIsPasting, checkClipboardForImage, isMacOS, pastePendingRef) => {
  ...
  const wrappedOnInput = (input: string, key: Key, event: InputEvent): void => {
    ...
    // On Windows Terminal/WSL, right-click paste may not trigger bracketed paste
    // mode. Fall back to detecting rapid multi-character input as paste.
    const isFromPaste =
      event.keypress.isPasted || (isWindows && !key.ctrl && !key.meta && !key.shift && input.length > 50);
