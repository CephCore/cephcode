 ...
 */
export async function writeRemoteAgentMetadata(taskId: string, metadata: RemoteAgentMetadata): Promise<void> {
 ...
 */
export function setInternalEventReader(reader: InternalEventReader, subagentReader: InternalEventReader): void {
...
class Project {
    ...
    return this.trackWrite(async () => {
      ...
      try {
        ...
        if (fileSize > MAX_TOMBSTONE_REWRITE_BYTES) {
          logForDebugging(`Skipping tombstone removal: session file too large (${formatFileSize(fileSize)})`, {
            level: 'warn',
          });
          return;
        }
  ...
  async insertFileHistorySnapshot(messageId: UUID, snapshot: FileHistorySnapshot, isSnapshotUpdate: boolean) {
    return this.trackWrite(async () => {
...
export async function recordContentReplacement(replacements: ContentReplacementRecord[], agentId?: AgentId) {
 ...
 */
export function getFirstMeaningfulUserMessageTextContent<T extends Message>(transcript: T[]): string | undefined {
...
function getEffectivePrUrl(originalPrUrl: string, prRepository: string, prNumber: number): string {
...
function renderPrUrlTemplate(template: string, prRepository: string, prNumber: number): string {
...
export async function loadFullLog(log: LogOption): Promise<LogOption> {
  ...
  try {
    ...
    return {
      ...
      contextCollapseSnapshot:
        sessionId && contextCollapseSnapshot?.sessionId === sessionId ? contextCollapseSnapshot : undefined,
      pinnedDate: sessionId ? pinnedDates.get(sessionId) : log.pinnedDate,
      pinnedGitStatus: sessionId ? pinnedGitStatuses.get(sessionId) : log.pinnedGitStatus,
    };
  } catch {
 ...
 */
export async function doesMessageExistInSession(sessionId: UUID, messageUuid: UUID): Promise<boolean> {
  const messageSet = await getSessionMessages(sessionId);
 ...
 */
export async function loadSubagentTranscripts(agentIds: string[]): Promise<{ [agentId: string]: Message[] }> {
 ...
 */
function transformMessagesForExternalTranscript(messages: Transcript, replIds: Set<string>): Transcript {
...
export function cleanMessagesForLogging(messages: Message[], allMessages: readonly Message[] = messages): Transcript {
 ...
 */
async function readLiteMetadata(filePath: string, fileSize: number, buf: Buffer): Promise<LiteMetadata> {
