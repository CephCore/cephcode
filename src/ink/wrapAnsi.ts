...
const wrapAnsi: (input: string, columns: number, options?: WrapAnsiOptions) => string = wrapAnsiBun
  ? (input, columns, options) =>
      needsJavaScriptWrap(input) ? wrapAnsiNpm(input, columns, options) : wrapAnsiBun(input, columns, options)
  : wrapAnsiNpm;
