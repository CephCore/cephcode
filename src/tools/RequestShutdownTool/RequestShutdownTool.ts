...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    reason: z
      .string()
      .optional()
      .describe('Optional explanation for why shutdown is being requested.'),
  }),
)
    ...
    if (!teammate) {
      return {
        data: {
          success: false,
          target,
          error: `Teammate "${target}" not found in team "${teamName}". Active teammates: ${Object.values(appState.teamContext?.teammates || {})
            .map(t => t.name)
