...
function filterSettingsEnv(env: Record<string, string> | undefined): Record<string, string> {
  const filtered = withoutCcdSpawnEnvKeys(withoutHostManagedProviderVars(withoutSSHTunnelVars(env)));
  // Strip NO_COLOR/FORCE_COLOR from settings.env — they control chalk/supports-color
  // and would strip Claude's own UI if set. They still reach subprocesses via subprocessEnv().
  const result: Record<string, string> = {};
  for (const [key, value] of Object.entries(filtered)) {
    const upper = key.toUpperCase();
    if (upper !== 'NO_COLOR' && upper !== 'FORCE_COLOR') {
      result[key] = value;
    }
  }
  return result;
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  if (ccdSpawnEnvKeys === undefined) {
    ccdSpawnEnvKeys =
      process.env.CLAUDE_CODE_ENTRYPOINT === 'claude-desktop' ? new Set(Object.keys(process.env)) : null;
  }
