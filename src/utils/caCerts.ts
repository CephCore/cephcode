...
export const getCACertificates = memoize((): string[] | undefined => {
  ...
  const useSystemCA =
    certStore !== 'bundled' &&
    (certStore === 'system' || !certStore || hasNodeOption('--use-system-ca') || hasNodeOption('--use-openssl-ca'));

  const extraCertsPath = process.env.NODE_EXTRA_CA_CERTS;

  logForDebugging(`CA certs: certStore=${certStore}, useSystemCA=${useSystemCA}, extraCertsPath=${extraCertsPath}`);
  ...
  if (useSystemCA) {
    ...
    const getCACerts = (tls as typeof tls & { getCACertificates?: (type: string) => string[] }).getCACertificates;
  ...
  if (extraCertsPath) {
    try {
      ...
      logForDebugging(`CA certs: Failed to read NODE_EXTRA_CA_CERTS file (${extraCertsPath}): ${error}`, {
        level: 'error',
      });
    }
  }
