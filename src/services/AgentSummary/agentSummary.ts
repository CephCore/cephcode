...
export function startAgentSummarization(
  ...
  async function runSummary(): Promise<void> {
    ...
    try {
      ...
      logForDebugging(`[AgentSummary] Forking for summary, ${cleanMessages.length} messages in context`);
      ...
      for (const msg of result.messages) {
        ...
        if (textBlock?.type === 'text' && textBlock.text.trim()) {
          ...
          if (summaryText === previousSummary) {
            idleSummaryCount++;
            logForDebugging(`[AgentSummary] Idle summary (x${idleSummaryCount}) — skipping redundant update`);
          } else {
  ...
  function scheduleNext(): void {
    if (stopped) return;
    const interval = idleSummaryCount > 0 ? SUMMARY_IDLE_INTERVAL_MS : SUMMARY_INTERVAL_MS;
