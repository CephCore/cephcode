...
const TIP_MATCHERS: TipMatcher[] = [
  {
    ...
    tip: {
      suggestion: 'Valid modes: "default", "plan", "acceptEdits", "bypassPermissions", or "auto"',
  ...
  {
    matches: (ctx): boolean => ctx.path === 'cleanupPeriodDays' && ctx.code === 'too_small' && ctx.expected === '0',
    tip: {
      suggestion:
  ...
  {
    matches: (ctx): boolean =>
      ctx.code === 'invalid_type' && ctx.expected === 'object' && ctx.received === null && ctx.path === '',
    tip: {
      suggestion:
  ...
  {
    matches: (ctx): boolean => ctx.path === 'permissions.additionalDirectories' && ctx.code === 'invalid_type',
    tip: {
      suggestion:
