...
export function assessSourceCredibility(url: string): SourceCredibility {
  try {
    ...
    if (
      OFFICIAL_DOMAINS.some((d) => domain === d || domain.endsWith("." + d))
    ) {
      score = 95;
      type = "official";
    ...
    // Check academic sources
      ...
      ACADEMIC_DOMAINS.some((d) => domain === d || domain.endsWith("." + d))
    ) {
      score = 90;
      type = "academic";
    ...
    // Check news sources
      ...
      NEWS_DOMAINS.some((d) => domain === d || domain.endsWith("." + d))
    ) {
      score = 75;
      type = "news";
    ...
    // Check community sources
      ...
      COMMUNITY_DOMAINS.some((d) => domain === d || domain.endsWith("." + d))
    ) {
      score = 65;
      type = "community";
    ...
    if (
      url.includes("blog") &&
      !OFFICIAL_DOMAINS.some((d) => domain.includes(d))
    ) {
...
export function detectConflicts(
  ...
  for (const stmt of statements) {
    for (const sentence of stmt.sentences) {
      ...
      const topicWords = sentenceWords
        .filter(w => queryWordsSet.has(w))
        .sort();
      
      if (topicWords.length < 1) {
        // Fallback: just use first 2 significant words if no query words match
        const significant = sentenceWords.filter(w => w.length > 5).slice(0, 2).sort();
...
function isSimilar(s1: string, s2: string): boolean {
    const w1 = new Set(s1.toLowerCase().split(/\s+/).filter(w => w.length > 4));
    const w2 = new Set(s2.toLowerCase().split(/\s+/).filter(w => w.length > 4));
...
export function generateTruthCheckSummary(
  ...
  const resolvedConflicts = conflicts.map((conflict) => {
    ...
    conflict.confidence =
      topClaim.avgCredibility > 80
        ? "high"
        : topClaim.avgCredibility > 60
          ? "medium"
          : "low";
  ...
  if (resolvedConflicts.length > 0) {
    recommendations.push(
      "Cross-reference conflicting claims with official documentation when possible.",
    );
    recommendations.push(
      "Pay attention to the credibility scores of sources when evaluating conflicting information.",
    );

    const lowCredibilityConflicts = resolvedConflicts.filter(
      (c) => c.confidence === "low",
    );
    if (lowCredibilityConflicts.length > 0) {
      recommendations.push(
        "Some conflicts have low confidence resolution - consider additional research.",
      );
    }
  }
  ...
  if (lowCredibilitySources.length > 0) {
    recommendations.push(
      `Be cautious with information from: ${lowCredibilitySources.map((s) => s.domain).join(", ")}`,
    );
  }
