...
export function clearDeliveredDiagnosticsForFile(fileUri: string): void {
  ...
  if (deliveredDiagnostics.has(normalizedUri)) {
    logForDebugging(`LSP Diagnostics: Clearing delivered diagnostics for ${normalizedUri}`);
  ...
  for (const [id, pending] of pendingDiagnostics.entries()) {
    ...
    if (pending.files.length === 0) {
      ...
      logForDebugging(`LSP Diagnostics: Filtered out edited file ${normalizedUri} from pending diagnostic ${id}`);
    }
  }
}
