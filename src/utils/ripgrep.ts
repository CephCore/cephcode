    ...
    const handleResult = (
      ...
      if (error.code === 'ENOENT' && !isFallback && !triedSystemRipgrepFallback) {
        ...
        if (systemPath !== 'rg') {
          ...
          ripGrepWithCommand(systemPath, args, target, abortSignal, (retryError, retryStdout, retryStderr) => {
...
const testRipgrepOnFirstUse = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    if (config.argv0) {
      ...
      const [stdout, code] = await Promise.all([(proc.stdout as unknown as Blob).text(), proc.exited]);
      test = {
        code,
        stdout,
      };
...
async function codesignRipgrepIfNecessary() {
  ...
  try {
    ...
    const quarantineResult = await execFileNoThrow('xattr', ['-d', 'com.apple.quarantine', builtinPath]);
