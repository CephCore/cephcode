...
export class SSETransport implements Transport {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logForDebugging(`SSETransport: HTTP ${response.status}${isPermanent ? ' (permanent)' : ''}`, {
          level: 'error',
        });
  ...
  private handleConnectionError(): void {
    ...
    if (elapsed < RECONNECT_GIVE_UP_MS) {
      ...
      const baseDelay = Math.min(RECONNECT_BASE_DELAY_MS * 2 ** (this.reconnectAttempts - 1), RECONNECT_MAX_DELAY_MS);
      // Add ±25% jitter
      ...
      logForDebugging(`SSETransport: Reconnection time budget exhausted after ${Math.round(elapsed / 1000)}s`, {
        level: 'error',
      });
  ...
  async write(message: StdoutMessage): Promise<void> {
    ...
    logForDebugging(`SSETransport: POST body keys=${Object.keys(message as Record<string, unknown>).join(',')}`);
