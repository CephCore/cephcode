...
async function moveVirtualCursor(page: Page, x: number, y: number, mode: 'pointer' | 'scroll' | 'text' = 'pointer') {
...
async function successResult(
  ...
  if (!opts?.skipScreenshot) {
    try {
      ...
      logForDebugging('BrowserTool: Screenshot failed or timed out: ' + error.message);
  ...
  try {
    ...
    switch (input.action) {
        ...
        try {
          ...
          if (healedUrl && healedUrl !== input.url) {
            ...
            return successResult(page, {
              skipScreenshot: true,
              extra: { content: `Healed: Original URL was broken, recovered via Google search to ${healedUrl}` },
            });
          }
      ...
      case 'click_role': {
        ...
        await page
          .getByRole(input.role as any, opts)
          .first()
          .click({ timeout });
        return successResult(page, { skipScreenshot: true });
      }
        ...
        const results = await page.evaluate((engineKey: string) => {
          ...
          const strategies: Record<string, Array<() => Result[]>> = {
            google: [
              ...
              () => {
                return Array.from(document.querySelectorAll('.g, .tF2Cxc'))
                  .map(c => {
                    const link = c.querySelector('a[href^="http"]') as HTMLAnchorElement;
                    ...
                    if (link && heading) {
                      return {
                        ...
                        snippet:
                          c
                            .querySelector('.VwiC3b, span.aCOpRe, div[data-sncf], div[role="heading"] + div')
                            ?.textContent?.trim() || '',
                      };
                    }
                    return null;
                  ...
                  .map(el => {
                    ...
                    if (link && heading) {
                      return {
                        title: heading.innerText.trim(),
                        link: link.href,
                        snippet: el.querySelector('.b_caption p, .b_algo p')?.textContent?.trim() || '',
                      };
                    }
                    return null;
                  })
            ...
            duckduckgo: [
              ...
              () => {
                return Array.from(document.querySelectorAll('article'))
                  .map(art => {
                    const link =
                      art.querySelector('a[data-testid="result-title-a"]') || art.querySelector('a[href^="http"]');
                    const heading = art.querySelector('h2');
                    if (link && heading) {
                      return {
                        title: heading.innerText.trim(),
                        link: (link as HTMLAnchorElement).href,
                        snippet: art.querySelector('div[data-testid="result-snippet"]')?.textContent?.trim() || '',
                      };
                    }
                    return null;
                  })
              ...
              () => {
                return Array.from(document.querySelectorAll('div[data-testid="cellInnerDiv"]'))
                  .map(t => {
                    ...
                    const text = t.querySelector('div[data-testid="tweetText"]') as HTMLElement;
                    if (link && text) {
                      return {
            ...
            reddit: [
              () => {
                const out: Result[] = [];
                const posts = Array.from(document.querySelectorAll('faceplate-tracker[source="search_results"]'));
                for (const p of posts) {
                  ...
                  if (link && (title as HTMLElement)) {
                    out.push({
                      ...
                      link: (link as HTMLAnchorElement).href,
                      snippet: p.querySelector('div[slot="text-body"]')?.textContent?.trim() || '',
                    });
                  }
                }
                return out;
            ...
            github: [
              () => {
                const out: Result[] = [];
                const titles = Array.from(document.querySelectorAll('div.search-title'));
            ...
            for (const h3 of h3s) {
              ...
              if (link && link.href && link.href.startsWith('http') && !seen.has(link.href)) {
                ...
                items.push({
                  ...
                  link: link.href,
                  snippet:
                    parent?.querySelector('p, span, div[class*="snippet"], div[class*="desc"]')?.textContent?.trim() ||
                    '',
                });
              }
            }
      ...
      case 'vision_map': {
        ...
        await page.evaluate(() => {
          ...
          document
            .querySelectorAll('.claude-vision-highlight')
            .forEach(el => el.classList.remove('claude-vision-highlight'));
        ...
        await page.evaluate(
          ({ msg }) => {
            try {
              ...
              document.querySelectorAll('.claude-neon-bar').forEach(b => b.classList.add('alarm'));
            ...
            `;
              ...
              document.getElementById('claude-resolved-btn')!.onclick = () => {
                ...
                if (el) el.remove();
                document.querySelectorAll('.claude-neon-bar').forEach(b => b.classList.remove('alarm'));
              };
