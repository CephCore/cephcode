...
export function HistorySearchDialog({ initialQuery, onSelect, onCancel }: Props): React.ReactNode {
  ...
  return (
    <FuzzyPicker
      ...
      onCancel={onCancel}
      emptyMessage={q_0 => (items === null ? 'Loading…' : q_0 ? 'No matching prompts' : 'No history yet')}
      selectAction="use"
      direction="up"
      ...
      renderItem={(item_2, isFocused) => (
        <Text>
          ...
          <Text color={isFocused ? 'suggestion' : undefined}> {truncateToWidth(item_2.firstLine, rowWidth)}</Text>
        </Text>
      )}
      renderPreview={item_3 => {
