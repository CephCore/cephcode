...
import { type GlobalConfig, getGlobalConfig, getRemoteControlAtStartup, saveGlobalConfig } from '../../utils/config.js';
...
export const ConfigTool = buildTool({
  ...
  async call({ setting, value }: Input, context): Promise<{ data: Output }> {
    ...
    if (feature('VOICE_MODE') && setting === 'voiceEnabled' && finalValue === true) {
      ...
      const { checkRecordingAvailability, checkVoiceDependencies, requestMicrophonePermission } = await import(
        '../../services/voice.js'
      );
