...
async function handleShutdownApproval(requestId: string, context: ToolUseContext): Promise<{ data: ResponseOutput }> {
...
async function handleShutdownRejection(requestId: string, reason: string): Promise<{ data: ResponseOutput }> {
  const teamName = getTeamName();
  const agentName = getAgentName() || 'teammate';
...
export const SendMessageTool: Tool<InputSchema, SendMessageToolOutput> = buildTool({
  ...
  async validateInput(input, _context) {
    ...
    if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'uds' && typeof input.message === 'string') {
