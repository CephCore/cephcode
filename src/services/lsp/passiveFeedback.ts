...
export function registerLSPNotificationHandlers(manager: LSPServerManager): HandlerRegistrationResult {
  ...
  for (const [serverName, serverInstance] of servers.entries()) {
    try {
      ...
      serverInstance.onNotification('textDocument/publishDiagnostics', (params: unknown) => {
        ...
        try {
          ...
          if (!params || typeof params !== 'object' || !('uri' in params) || !('diagnostics' in params)) {
            ...
            logError(err);
            logForDebugging(`Invalid diagnostic params from ${serverName}: ${jsonStringify(params)}`);
            return;
          }
          ...
          const err = toError(error);
          logError(err);
          logForDebugging(`Unexpected error processing diagnostics from ${serverName}: ${err.message}`);
      ...
      logError(err);
      logForDebugging(`Failed to register diagnostics handler for ${serverName}: ` + `Error: ${err.message}`);
    }
  }
  ...
  if (registrationErrors.length > 0) {
    ...
    logError(
      new Error(`Failed to register diagnostics for ${registrationErrors.length} LSP server(s): ${failedServers}`),
    );
    logForDebugging(
