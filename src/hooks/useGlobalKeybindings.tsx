...
type Props = {
  ...
  onShowTranscriptHelp?: () => void;
  onJumpPrevPrompt?: () => void;
  onJumpNextPrompt?: () => void;
};
...
export function GlobalKeybindingHandlers({
  ...
  // Transcript help overlay (?) — shows available shortcuts
  const handleShowTranscriptHelp = useCallback(() => {
    onShowTranscriptHelp?.();
  }, [onShowTranscriptHelp]);
  useKeybinding('transcript:showHelp', handleShowTranscriptHelp, {
    context: 'Transcript',
    isActive: isInTranscript,
  });

  // Jump to previous user prompt ({) in transcript
  const handleJumpPrevPrompt = useCallback(() => {
    onJumpPrevPrompt?.();
  }, [onJumpPrevPrompt]);
  useKeybinding('transcript:jumpPrevPrompt', handleJumpPrevPrompt, {
    context: 'Transcript',
    isActive: isInTranscript,
  });

  // Jump to next user prompt (}) in transcript
  const handleJumpNextPrompt = useCallback(() => {
    onJumpNextPrompt?.();
  }, [onJumpNextPrompt]);
  useKeybinding('transcript:jumpNextPrompt', handleJumpNextPrompt, {
    context: 'Transcript',
    isActive: isInTranscript,
  });

  // Toggle shortcuts panel (v) in transcript
  const handleToggleShortcuts = useCallback(() => {
    onShowTranscriptHelp?.();
  }, [onShowTranscriptHelp]);
  useKeybinding('transcript:toggleShortcuts', handleToggleShortcuts, {
    context: 'Transcript',
    isActive: isInTranscript,
  });
