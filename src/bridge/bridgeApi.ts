...
function handleErrorStatus(status: number, data: unknown, context: string, clearTrustedDeviceToken?: () => void): void {
...
export function isExpiredErrorType(errorType: string | undefined): boolean {
  ...
  return (
    errorType.includes('expired') || errorType.includes('lifetime') || errorType.includes('stale') // stale_session → trusted device token needs /login
  );
}
...
export function isSuppressible403(err: BridgeFatalError): boolean {
  ...
  return err.message.includes('external_poll_sessions') || err.message.includes('environments:manage');
}
