...
import { initializePolicyLimitsLoadingPromise, isPolicyLimitsEligible } from '../services/policyLimits/index.js';
import {
  ...
  if (isEligibleForRemoteManagedSettings()) {
    ...
    if (getIsNonInteractiveSession() && isBetaTracingEnabled()) {
      void doInitializeTelemetry().catch(error => {
        logForDebugging(`[3P telemetry] Eager telemetry init failed (beta tracing): ${errorMessage(error)}`, {
          level: 'error',
        });
      });
    }
    ...
    void waitForRemoteManagedSettingsToLoad()
      ...
      .catch(error => {
        logForDebugging(`[3P telemetry] Telemetry init failed (remote settings path): ${errorMessage(error)}`, {
          level: 'error',
        });
      });
  } else {
