...
/** Build a short human-readable summary string from context analysis data. */
function formatContextSummary(data: any): string {
  const used = formatTokens(data.totalTokens);
  const limit = formatTokens(data.maxTokens);
...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext): Promise<React.ReactNode> {
  ...
  // wasting ~1.6k tokens per /context call. The visualization grid is purely
  // for human inspection and offers no value to the model.
  const summary = formatContextSummary(data);
