...
async function loadOutputStyleFromFile(
  ...
  try {
    ...
    const forceForPlugin =
      forceRaw === true || forceRaw === 'true' ? true : forceRaw === false || forceRaw === 'false' ? false : undefined;
  ...
  for (const plugin of enabled) {
    ...
    if (plugin.outputStylesPath) {
      try {
        ...
        if (styles.length > 0) {
          logForDebugging(`Loaded ${styles.length} output styles from plugin ${plugin.name} default directory`);
        }
      } catch (error) {
        logForDebugging(`Failed to load output styles from plugin ${plugin.name} default directory: ${error}`, {
          level: 'error',
        });
      }
    }
        ...
        try {
          ...
          if (stats.isDirectory()) {
            ...
            if (style) {
              ...
              logForDebugging(`Loaded output style from plugin ${plugin.name} custom file: ${stylePath}`);
            }
          }
        } catch (error) {
          logForDebugging(
