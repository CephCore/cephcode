...
export function isCellEmpty(screen: Screen, cell: Cell): boolean {
  ...
  return (
    cell.char === ' ' && cell.styleId === screen.emptyStyleId && cell.width === CellWidth.Narrow && !cell.hyperlink
  );
}
...
function diffSameWidth(
  ...
  for (let y = startY; y < endY; y++) {
    ...
    if (prevIn && nextIn) {
      if (diffRowBoth(prevCells, nextCells, prev, next, rowCI, y, startX, rowEndX, prevCell, nextCell, cb)) return true;
    } else if (prevIn) {
