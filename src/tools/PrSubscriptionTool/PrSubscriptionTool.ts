...
import {
  SUBSCRIBE_PR_ACTIVITY_TOOL_NAME,
  UNSUBSCRIBE_PR_ACTIVITY_TOOL_NAME,
} from './constants.js'
...
async function checkPrActivity(
  sub: PrSubscription,
): Promise<{ hasUpdate: boolean; summary: string }> {
  ...
  try {
    ...
    if (reviewCount !== sub.lastReviewCount) {
      ...
      if (newReviews > 0) {
        updates.push(
          `${newReviews} new review(s) (total: ${reviewCount})`,
        )
      }
    }
    ...
    logForDebugging(
      `[PrSubscription] checkPrActivity failed for ${owner}/${repo}#${prNumber}: ${String(err)}`,
    )
...
const subscribeInputSchema = lazySchema(() =>
  z.strictObject({
    pr_url: z
      .string()
      .describe('Full GitHub PR URL to watch (e.g., https://github.com/owner/repo/pull/123)'),
  }),
)
...
const unsubscribeInputSchema = lazySchema(() =>
  z.strictObject({
    pr_url: z
      .string()
      .describe('Full GitHub PR URL to stop watching'),
  }),
)
 ...
 */
export const SubscribePrActivityTool: Tool<SubscribeInputSchema, SubscribeOutput> =
  buildTool({
    ...
    async call(input, _context) {
      ...
      sub.pollInterval = setInterval(() => {
        void (async () => {
          try {
            ...
            if (hasUpdate) {
              logForDebugging(
                `[PrSubscription] PR activity for ${prUrl}: ${summary}`,
              )
      ...
      logForDebugging(
        `[PrSubscription] Subscribed to ${prUrl} (checks every 60s)`,
      )
 ...
 */
export const UnsubscribePrActivityTool: Tool<
