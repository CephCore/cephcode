...
export function parseSkillFrontmatterFields(
  ...
  const userInvocable =
    frontmatter['user-invocable'] === undefined ? true : parseBooleanFrontmatter(frontmatter['user-invocable']);
...
export const getSkillDirCommands = memoize(async (cwd: string): Promise<Command[]> => {
  ...
  const [managedSkills, userSkills, projectSkillsNested, additionalSkillsNested, simpleProjectSkills, legacyCommands] =
    await Promise.all([
      ...
      projectSettingsEnabled && !skillsLocked ? loadProjectSkills(cwd) : Promise.resolve([]),
...
export { clearSkillCaches as clearCommandCaches, getSkillDirCommands as getCommandDirCommands, transformSkillFiles };
...
export async function addSkillDirectories(dirs: string[]): Promise<void> {
  ...
  if (newSkillCount > 0) {
    ...
    logForDebugging(`[skills] Dynamically discovered ${newSkillCount} skills from ${dirs.length} directories`);
