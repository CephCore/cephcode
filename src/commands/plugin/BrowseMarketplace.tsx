    ...
    return (
      <Box flexDirection="column">
        ...
        {marketplaces.map((marketplace_3, index) => (
          <Box key={marketplace_3.name} flexDirection="column" marginBottom={index < marketplaces.length - 1 ? 1 : 0}>
            <Box>
            ...
            <Box marginLeft={2}>
              <Text dimColor>
                {marketplace_3.totalPlugins} {plural(marketplace_3.totalPlugins, 'plugin')} available
      ...
      <Box flexDirection="column">
        ...
        <Box flexDirection="column" marginBottom={1}>
          ...
          {selectedPlugin.entry.lspServers && (
            <Text dimColor>
              · LSP Servers:{' '}
              {Array.isArray(selectedPlugin.entry.lspServers)
                ? selectedPlugin.entry.lspServers.join(', ')
                : typeof selectedPlugin.entry.lspServers === 'object'
                  ? Object.keys(selectedPlugin.entry.lspServers).join(', ')
                  : 'configured'}
            </Text>
          )}
          {!selectedPlugin.entry.commands &&
  ...
  return (
    <Box flexDirection="column">
      ...
      {visiblePlugins.map((plugin_6, visibleIndex) => {
        ...
        return (
          <Box key={plugin_6.pluginId} flexDirection="column" marginBottom={isLast && !error ? 0 : 1}>
            <Box>
            ...
            {plugin_6.entry.description && (
              <Box marginLeft={4}>
                ...
                {plugin_6.entry.contextCost?.perTurn && (
                  <Text dimColor> · ~{plugin_6.entry.contextCost.perTurn} tok/turn</Text>
                )}
              </Box>
            )}
          </Box>
