...
export async function prepareForkedCommandContext(
  ...
  const baseAgent =
    agents.find(a => a.agentType === agentTypeName) ?? agents.find(a => a.agentType === 'general-purpose') ?? agents[0];
 ...
 */
export function extractResultText(agentMessages: Message[], defaultText = 'Execution completed'): string {
...
function logForkAgentQueryEvent({
  ...
  const totalInputTokens =
    totalUsage.input_tokens + totalUsage.cache_creation_input_tokens + totalUsage.cache_read_input_tokens;
