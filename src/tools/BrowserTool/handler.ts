...
export async function handleBrowserAction(input: BrowserActionInput): Promise<BrowserResult> {
  const { page, context } = await getBrowser(input);
    ...
    switch (input.action) {
      ...
      case 'click_at':
        if (typeof input.x !== 'number' || typeof input.y !== 'number')
          throw new Error('x and y required for click_at');
      ...
      case 'type_at':
        if (typeof input.x !== 'number' || typeof input.y !== 'number' || !input.text)
          throw new Error('x + y + text required');
      ...
      case 'open_new_tab':
        if (browserContext) {
          ...
          if (input.url) {
            await pageInstance.goto(input.url, {
              waitUntil: 'networkidle',
              timeout: input.timeout || DEFAULT_NAVIGATION_TIMEOUT_MS,
            });
          }
...
function shouldRunHeadless(input?: BrowserActionInput): boolean {
  ...
  return ['1', 'true'].includes((process.env.BROWSER_TOOL_HEADLESS || '').toLowerCase());
}
